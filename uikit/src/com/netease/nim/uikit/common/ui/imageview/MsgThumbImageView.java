...
public class MsgThumbImageView extends ImageView {
    ...
    public void loadAsUrl(final String url, final int width, final int height, final int maskId, final String ext) {
        ...
        RequestBuilder builder;
        if (ImageUtil.isGif(ext)) {
            builder = Glide.with(getContext().getApplicationContext()).asGif().load(Uri.parse(url));
        } else {
            RequestOptions options = new RequestOptions()
                    .override(width, height)
                    .centerCrop()
                    .placeholder(R.drawable.nim_image_default)
                    .error(R.drawable.nim_image_default)
                    .transform(new RoundedCorners((int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 10, getResources().getDisplayMetrics())));
