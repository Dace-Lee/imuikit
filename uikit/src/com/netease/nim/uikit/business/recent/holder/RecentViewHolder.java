...
public abstract class RecentViewHolder extends RecyclerViewHolder<BaseQuickAdapter, BaseViewHolder, RecentContact> {
    ...
    public void inflate(BaseViewHolder holder, final RecentContact recent) {
        holder.addOnClickListener(R.id.img_head);
    ...
    protected void loadPortrait(final RecentContact recent) {
        final String sessionId = recent.getContactId();
        // 设置头像
        if (recent.getSessionType() == SessionTypeEnum.P2P) {
            ImUserRespDTO dto = ImUserRespCache.Companion.getInstance().get(sessionId);
            if (dto != null && !dto.getDataMasking() && dto.getMemberLevel() != 1) {
                imgHead.loadAvatar(dto.getAvatar());
            } else {
    ...
    protected void updateNickLabel(RecentContact recentContact) {
        String name = UserInfoHelper.getUserTitleName(recentContact.getContactId(), recentContact.getSessionType());
        String userInfo = "";
        if (adapter.isPurchaseSupporter) {
            if (recentContact.getSessionType() == SessionTypeEnum.P2P) {
                String sign = UserInfoHelper.getSign(recentContact.getContactId());
                if (sign != null) {
                    userInfo = sign;
                }
            }
        ...
        tvNickname.setText(name + "  " + userInfo);
    }
