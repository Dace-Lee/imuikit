...
import static com.netease.nim.uikit.business.uinfo.ImUserRespCache.Callback;
import static com.netease.nim.uikit.business.uinfo.ImUserRespCache.Companion;
...
public class RecentContactsFragment extends TFragment {
    ...
    public static final String IS_PURCHASE_SUPPORTER = "is_purchase_supporter";
    private boolean isPurchaseSupporter;
    ...
    public void onActivityCreated(Bundle savedInstanceState) {
        ...
        if (bundle != null) {
            isPurchaseSupporter = bundle.getBoolean(IS_PURCHASE_SUPPORTER, false);
        }
    ...
    private void initMessageList() {
        ...
        // adapter
        adapter = new RecentContactAdapter(recyclerView, items, isPurchaseSupporter);
        initCallBack();
    ...
    private SimpleClickListener<RecentContactAdapter> touchListener = new SimpleClickListener<RecentContactAdapter>() {
        ...
        public void onItemChildClick(RecentContactAdapter adapter, View view, int position) {
            ...
            if (view.getId() == R.id.img_head) {
                boolean redirected = ImUserRespCache.instance.previewProfile(getContext(), recent.getContactId());
                if (!redirected) {
                    if (callback != null) {
                        callback.onItemClick(recent);
                    }
                ...
                NIMClient.getService(MsgService.class).queryRecentContacts().setCallback(new RequestCallbackWrapper<List<RecentContact>>() {
                    ...
                    public void onResult(int code, List<RecentContact> recents, Throwable exception) {
                        ...
                        for (RecentContact loadedRecent : loadedRecents) {
                            ...
                            } else if (loadedRecent.getSessionType() == SessionTypeEnum.P2P) {
                                userIdList.add(loadedRecent.getContactId());
                            }
                        }
                        // 此处如果是界面刚初始化，为了防止界面卡顿，可先在后台把需要显示的用户资料和群组资料在后台加载好，然后再刷新界面
                        if (!userIdList.isEmpty()) {
                            Companion.getInstance().load(userIdList, new Callback() {
                                @Override
                                public void onDone() {
                                    msgLoaded = true;
                                    if (isAdded()) {
