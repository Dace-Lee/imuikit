    ...
    private void setHeadImageView() {
        ...
        if (isMiddleItem()) {
            ...
            if (isReceivedMessage()) {
                ImUserRespDTO dto = ImUserRespCache.Companion.getInstance().get(message.getSessionId());
                if (dto != null && !dto.getDataMasking() && dto.getMemberLevel() != 1) {
                    show.loadAvatar(dto.getAvatar());
                } else {
                    show.loadBuddyAvatar(message);
                }
            } else { // is me
                show.loadAvatar(ImUserRespCache.Companion.getInstance().getCurrentUserAvatar());
//                show.loadBuddyAvatar(message);
            }
        }
