...
public class VideoMessageHelper {
    ...
    public int getCaptureRequestCode() {
        return captureRequestCode;
    }
    ...
    public interface VideoMessageHelperListener {
        ...
        void onPicturePicked(File file, String md5);
    }

    /**
     * 显示视频拍摄或从本地相册中选取
     */
    public void showVideoSource(int local, int capture, boolean canLongClickTakeVideo) {
...
//        dialog.show();
        ...
        chooseVideoFromCamera(canLongClickTakeVideo);
    }
     ...
     * 拍摄视频
     */
    protected void chooseVideoFromCamera(boolean canLongClickTakeVideo) {
        ...
        // 启动视频录制
        CaptureVideoActivity.start(activity, videoFilePath, captureRequestCode, canLongClickTakeVideo);
    }
    ...
    public void onCaptureVideoResult(Intent data) {
        ...
        if (duration == 0) {
            if (listener != null) {
                listener.onPicturePicked(new File(videoPath), md5);
            }
